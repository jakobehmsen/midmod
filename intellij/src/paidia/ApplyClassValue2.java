...
public class ApplyClassValue2 extends AbstractValue2 implements Value2Observer {
    private ClassValue value;
    private Supplier<Value2> argumentProvider;
    private Hashtable<String, Value2> arguments;

    public ApplyClassValue2(ClassValue value, Supplier<Value2> argumentProvider) {
        this.value = value;
        value.addObserver(this);
        arguments = new Hashtable<>();
        this.argumentProvider= argumentProvider;

        value.getClassParameters().forEach(x -> {
            arguments.put(x.getId(), argumentProvider.get());
        });
    }
    ...
    public ViewBinding2 toView(PlaygroundView playgroundView) {
        ...
        Runnable addArguments = () -> {
            // Transform selector into application

            Value2 selectorAsApplication = value.getSelectorAsApplication();
            view.add(selectorAsApplication.toView(playgroundView).getComponent());
            ...
            /*value.getClassParameters().forEach(x -> {
                ...
                Value2 value = arguments.get(x.getId());
